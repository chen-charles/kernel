...
"""
CC	=	g++ 
CFLAGS	=	-w -m32 -masm=intel -std=c++11 -fno-exceptions -fpermissive -Wno-unused -O2 -Wall -Wextra -c
"""
# -ffreestanding   -fno-rtti 
...
all_name = []
c_data = []
for name, fullname, root in walk(path, "cpp"):
	inc = ["%s "%fullname]
...
for name, fullname, root in walk(path, "cc"):
	inc = ["%s "%fullname]
	all_name.append(fullname[:-3]+".o")
	for i in parse_include(root, name, sep):
		inc.append("%s "%i)
	s = "%s.o: %s\n\t$(CC) $(CFLAGS) $< -o $@\n"%(fullname[:-4], "".join(inc))
	c_data.append(s)
